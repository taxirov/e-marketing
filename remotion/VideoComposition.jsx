    ...
    <AbsoluteFill style={{ backgroundColor: '#0f172a' }}>
      ...
      <AbsoluteFill style={slideContainerStyle}>
        {safeSlides.map((slide, index) => {
          ...
          const duration = index === safeSlides.length - 1 ? Math.max(1, durationInFrames - from) : slideDuration
          ...
          return (
            <Sequence key={`slide-${index}`} from={from} durationInFrames={duration}>
              <AbsoluteFill style={{ justifyContent: 'center', alignItems: 'center' }}>
                {slide?.src && (
                  <Img src={slide.src} style={{ ...imageStyle, opacity }} />
                )}
              </AbsoluteFill>
            </Sequence>
          )
